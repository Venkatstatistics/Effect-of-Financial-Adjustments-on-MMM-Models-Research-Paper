...
lm.aryma <- function(formula, data, ..., newdata = NULL, media_vars) {
  model <- lm(formula = formula, data = data, ...)
  
  # Predictions and actuals
  if (is.null(newdata)) {
    preds <- predict(model, data)
    actuals <- model.response(model.frame(model))
  } else {
    preds <- predict(model, newdata = newdata)
    actuals <- newdata[[all.vars(formula)[1]]]
  }
  
  # Compute MAPE
  if (any(actuals == 0)) {
    mape <- NA
    warning("MAPE is not defined when actual values contain zero.")
  } else {
    mape <- mean(abs((actuals - preds) / actuals)) * 100
  }
  
  # Compute NRMSE
  nrmse <- (mean((preds - actuals)^2)^0.5) / (max(actuals) - min(actuals))
  
  # Compute averages for independent variables
  model_frame <- as.data.frame(model.frame(model))
  data_X <- model_frame %>% select(-y) %>% mutate(`(Intercept)` = 1) %>% select(`(Intercept)`, everything())
  
  data_avg <- data.frame(
    Variable = colnames(data_X),
    Average = colMeans(data_X),
    row.names = NULL
  )
  ...
  # Combine coefficients and averages and compute contributions
  beta_into_X <- left_join(data_avg, coef_df, by = "Variable") %>% 
    mutate(Coefficient = ifelse(!is.na(Coefficient), Coefficient, 1),
           Beta_into_X = Coefficient * Average,
           Abs_Beta_into_X = abs(Beta_into_X),
           Non_Abs_Contributions = round((Beta_into_X / sum(Beta_into_X)) * 100, 2),
           Contribution = round((Abs_Beta_into_X / sum(Abs_Beta_into_X)) * 100, 2),
           Sign = sign(Coefficient),
           Contribution_correct_sign = round(Contribution * Sign, 2),
           `Clean variable name` = sapply(Variable, clean_varnames))
  
  contrib_tbl <- beta_into_X %>%
    select(`Clean variable name`, Contribution_correct_sign) %>%
    rename(Contributions = Contribution_correct_sign) %>%
    arrange(desc(Contributions))  # Arrange in descending order
  
  rownames(contrib_tbl) <- contrib_tbl$`Clean variable name`
  contrib_tbl$`Clean variable name` <- NULL
  contrib_tbl$Contributions <- sprintf("%.2f %%", contrib_tbl$Contributions)
  
  # Contribution Plot
  plot <- beta_into_X %>%
    mutate(Variable = fct_reorder(Variable, !!sym("Contribution_correct_sign"))) %>%
    ggplot(aes(x = Variable, y = !!sym("Contribution_correct_sign"))) +
    geom_col(fill = "#002060", width = 0.9) +
    geom_text(aes(label = !!sym("Contribution_correct_sign")), 
              hjust = ifelse(beta_into_X[["Contribution_correct_sign"]] > 0, -0.1, 1.0), size = 4.5) +
    labs(title = "Absolute Summing upto 100 Contribution Chart", x = "Variables", y = "Contribution%") +
    theme_classic() +
    theme(
  ...
  # Decomp.RSSD Computation
  # Compute Spend Share
  spend_data <- data[media_vars]
  total_spends <- colSums(spend_data)
  grand_total_spends <- sum(total_spends)
  spend_share <- data.frame(
    `Clean variable name` = media_vars,
    `Spend Share` = total_spends / grand_total_spends,
    row.names = NULL, check.names = FALSE
  )
  
  # Compute Effect Share
  effect_share <- beta_into_X %>%
    filter(`Clean variable name` %in% media_vars) %>%
    select(`Clean variable name`, Beta_into_X) %>%
    mutate(
      `Clean variable name` = `Clean variable name`,
      `Effect Share` = Beta_into_X / sum(Beta_into_X)
    )
  
  # Merge Spend Share and Effect Share
  decomp_df <- left_join(spend_share, effect_share, by = "Clean variable name")
  decomp_df[is.na(decomp_df)] <- 0
  decomp_df$Diff <- (decomp_df$`Effect Share` - decomp_df$`Spend Share`)^2
  
  # Compute DECOMP.RSSD
  decomp_rssd <- sqrt(sum(decomp_df$Diff))
  
  model$contrib_compute_df <- beta_into_X
  model$contrib_tbl <- contrib_tbl
  model$mape <- mape
  model$nrmse <- nrmse
  model$contrib_plot <- plot
  model$decomp_df <- decomp_df
  model$decomp.rssd <- decomp_rssd
  
  class(model) <- c("lm.aryma", "lm")
  return(model)
}

summary.lm.aryma <- function(object, ...) {
  summ <- NextMethod("summary")
  summ$MAPE               <- object$mape
  summ$NRMSE              <- object$nrmse
  summ$contribution       <- object$contrib_tbl
  summ$Decomp.RSSD        <- object$decomp.rssd
  
  class(summ) <- c("summary.lm.aryma", class(summ))
  print(summ)
  
  cat(sprintf("MAPE: %.2f%%    NRMSE: %.5f    Decomp.RSSD: %.2f\n\n", object$mape, object$nrmse, object$decomp.rssd))
  cat('Model Contributions (Abs. summing to 100):\n')
  
  print(object$contrib_plot)
  print(object$contrib_tbl)
  
  # Compute VIFs
  vifs <- data.frame(VIF = car::vif(object))
  summ$VIF <- vifs
